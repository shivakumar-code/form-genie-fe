...
const ApplicationForm = () => {
    ...
    try {
      ...
      try {
        const response = await axios.post("http://localhost:5000/api/auth/send-otp", {
          cardNumber: formData.cardNumber || 'Card123',
          imgSrc
        });
  ...
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/auth/verify-otp-fetch-data", {
        cardNumber: extractedText? extractedText:'Card123',
        otp
      });
      alert("OTP Verified!");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/auth/send-otp", {
        citizenId: formData.idNumber || 'Card123',
